...
public class PointController {
    ...
    public ResponseEntity<Map<String, Object>> pedometerCheck(@RequestParam("file") MultipartFile file) {
        ...
        String systemPrompt = "당신은 만보기 데이터를 확인하는 AI 비서입니다. 이미지를 인식하고, 만보기 캡쳐일 경우 해당 이미지에서 걸음 수를 추출해줍니다. 만약 만보기 사진이 아니라면 모두 -1를 반환합니다.";
        String userPrompt = "이 이미지에서 걸음 수를 숫자로만 추출해 주세요. 만보기 사진이 아닐 경우 반드시 -1을 반환해 주세요.";
        String stepCountStr = openAIService.sendImageToGPT(file, systemPrompt, userPrompt);
        if (stepCountStr.equals("-1")) {
            return ResponseEntity.ok(Map.of("message", "만보기 사진을 첨부해 주세요.", "result", false));
        }

        int stepCount = Integer.parseInt(stepCountStr.replaceAll("[^0-9]", ""));
        System.out.println("=================> " + stepCount);
        if (stepCount < 1000) {
            return ResponseEntity.ok(Map.of("message", "걸음 수 부족. 포인트가 적립되지 않았습니다.", "result", false));
        }
        ...
        return ResponseEntity.ok(Map.of("result", true, "message", "만보기 포인트 적립 완료!", "point", points));
    }
